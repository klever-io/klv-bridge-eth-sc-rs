...
// Correction for transaction simulation, to avoid issues with block timestamp
const TIME_SIMULATE_BLOCK_CORRECTION: u64 = 5; // seconds

#[klever_sc::contract]
pub trait PriceAggregator:
    klever_sc_modules::pause::PauseModule + staking_module::StakingModule + events::EventsModule
{
    #[init]
    fn init(
        &self,
    ...
    fn require_valid_submission_timestamp(&self, submission_timestamp: u64) {
        let current_timestamp = self.blockchain().get_block_timestamp() + TIME_SIMULATE_BLOCK_CORRECTION;
        require!(
