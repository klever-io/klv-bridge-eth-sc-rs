    ...
    + klever_sc_modules::pause_admin::PauseAdminModule
{
    ...
    fn add_refund_batch(&self, refund_transactions: ManagedVec<Transaction<Self::Api>>) {
        ...
        let refund_payments = self.call_value().all_kda_transfers().deref().clone();
        require!(
    ...
    fn create_transaction(
        ...
        let tx = Transaction {
            ...
            amount: eth_side_amount.clone(), // Store ETH-side amount in transaction
            is_refund_tx: false,
        };
    ...
    fn claim_refund(&self, token_id: TokenIdentifier) -> KdaTokenPayment<Self::Api> {
        let caller = self.blockchain().get_caller();
        ...
        self.tx()
            .to(ToCaller)
            .single_kda(&token_id, 0, &refund_amount)
            .transfer();
    ...
    fn withdraw_refund_fees_for_ethereum(
        ...
        self.tx()
            .to(multisig_owner)
            .single_kda(&token_id, 0, &amount_out)
            .transfer();
    ...
    fn compute_total_amounts_from_index(
        ...
        for index in start_index..end_index {
            ...
            for tx in last_batch.iter() {
                let new_payment = KdaTokenPayment::new(tx.token_identifier, 0, tx.amount);
