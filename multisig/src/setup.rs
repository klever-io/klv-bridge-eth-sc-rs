    ...
    + klever_sc_modules::pause::PauseModule
{
    ...
    /// Maps an KDA token to an ERC20 address. Used by relayers.
    #[only_owner]
    #[endpoint(addMapping)]
    ...
    #[only_owner]
    #[endpoint(pauseKdaSafe)]
    fn pause_kda_safe(&self) {
    ...
    #[only_owner]
    #[endpoint(unpauseKdaSafe)]
    fn unpause_kda_safe(&self) {
        let kda_safe_addr = self.kda_safe_address().get();
    ...
    #[only_owner]
    #[payable("*")]
    #[endpoint(initSupplyKdaSafe)]
    fn init_supply_kda_safe(&self, token_id: TokenIdentifier, amount: BigUint) {
        let kda_safe_addr = self.kda_safe_address().get();
    ...
    #[only_owner]
    #[endpoint(initSupplyMintBurnKdaSafe)]
    fn init_supply_mint_burn_kda_safe(
        &self,
    ...
    /// This is used in the KdaSafe contract to determine the fee amount
    ///
    /// fee_amount = eth_gas_limit * price_per_gas_unit
    ///
    /// where price_per_gas_unit is queried from the aggregator (fee estimator SC)
    #[only_owner]
    #[endpoint(changeKlvToEthGasLimit)]
    fn change_klv_to_eth_gas_limit(&self, new_gas_limit: BigUint) {
        let kda_safe_addr = self.kda_safe_address().get();
    ...
    #[only_owner]
    #[endpoint(kdaSafeAddTokenToWhitelist)]
    fn kda_safe_add_token_to_whitelist(
        &self,
        token_id: &TokenIdentifier,
    ...
    #[only_owner]
    #[endpoint(setMultiTransferOnKdaSafe)]
    fn set_multi_transfer_on_kda_safe(&self) {
        let multi_transfer_kda_address = self.multi_transfer_kda_address().get();
        let kda_safe_addr = self.kda_safe_address().get();
        self.tx()
            .to(kda_safe_addr)
            .typed(kda_safe_proxy::KDASafeProxy)
            .set_multi_transfer_contract_address(OptionalValue::Some(multi_transfer_kda_address))
            .sync_call();
    }

    #[only_owner]
    #[endpoint(setKdaSafeOnMultiTransfer)]
    fn set_kda_safe_on_multi_transfer(&self) {
        let kda_safe_address = self.kda_safe_address().get();
        ...
        self.tx()
            ...
            .set_kda_safe_contract_address(OptionalValue::Some(kda_safe_address))
            .sync_call();
    }

    #[only_owner]
    #[endpoint(kdaSafeRemoveTokenFromWhitelist)]
    fn kda_safe_remove_token_from_whitelist(&self, token_id: TokenIdentifier) {
        let kda_safe_addr = self.kda_safe_address().get();
        self.tx()
            .to(kda_safe_addr)
            .typed(kda_safe_proxy::KDASafeProxy)
    ...
    /// Sets maximum batch size for the KdaSafe SC.
    ...
    #[only_owner]
    #[endpoint(kdaSafeSetMaxTxBatchSize)]
    fn kda_safe_set_max_tx_batch_size(&self, new_max_tx_batch_size: usize) {
        let kda_safe_addr = self.kda_safe_address().get();
        self.tx()
            .to(kda_safe_addr)
            .typed(kda_safe_proxy::KDASafeProxy)
    ...
    /// Sets the maximum block duration in which an KdaSafe batch accepts transactions
    ...
    #[only_owner]
    #[endpoint(kdaSafeSetMaxTxBatchBlockDuration)]
    fn kda_safe_set_max_tx_batch_block_duration(&self, new_max_tx_batch_block_duration: u64) {
        let kda_safe_addr = self.kda_safe_address().get();
    ...
    /// Sets the maximum bridged amount for the token for the Klever Blockchain -> Ethereum direction.
    /// Any attempt to transfer over this amount will be rejected.
    #[only_owner]
    #[endpoint(kdaSafeSetMaxBridgedAmountForToken)]
    fn kda_safe_set_max_bridged_amount_for_token(
        &self,
        token_id: TokenIdentifier,
        max_amount: BigUint,
    ) {
    ...
    /// Same as the function above, but for Ethereum -> Klever Blockchain transactions.
    #[only_owner]
    #[endpoint(multiTransferKdaSetMaxBridgedAmountForToken)]
    fn multi_transfer_kda_set_max_bridged_amount_for_token(
        &self,
        token_id: TokenIdentifier,
        max_amount: BigUint,
    ) {
    ...
    /// Any failed Ethereum -> Klever Blockchain transactions are added into so-called "refund batches"
    /// This configures the size of a batch.
    #[only_owner]
    #[endpoint(multiTransferKdaSetMaxRefundTxBatchSize)]
    fn multi_transfer_kda_set_max_refund_tx_batch_size(&self, new_max_tx_batch_size: usize) {
        let multi_transfer_kda_addr = self.multi_transfer_kda_address().get();
    ...
    #[only_owner]
    #[endpoint(multiTransferKdaSetMaxRefundTxBatchBlockDuration)]
    fn multi_transfer_kda_set_max_refund_tx_batch_block_duration(
        &self,
    ...
    #[only_owner]
    #[endpoint(multiTransferKdaSetWrappingContractAddress)]
    fn multi_transfer_kda_set_wrapping_contract_address(
        &self,
